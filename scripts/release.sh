...
function generate_changelog() {
  release_version=$1
  log=$(git log --pretty=format:'  * [%h](https://github.com/pactflow/terraform/commit/%h) - %s (%an, %ad)' ${release_version}..HEAD | egrep -v "wip(:|\()" | grep -v "docs(" | grep -v "chore(" | grep -v Merge | grep -v "test(")
